    ...
    async pushLatestLocation(iotSerialNumber, latestCoordinates) {
        ...
        try {
            ...
            if (!activeTrackingRoute) {
                ...
                const trackingRoute = await trackingRouteService.addTrackingRoute(trackingData, { transaction });

                // Stores current gps coordinates with tracking route id
                await locationService.pushLatestLocation(activeWearable.id, latestCoordinates, trackingRoute.id, { transaction });
            ...
            if (trackingType === "stationary") {
                ...
                if (distanceFromReferencePoint < MOVEMENT_THRESHOLD) {
                    ...
                    // Still stationary
                    await trackingRouteService.updateAccumulatedDistance(trackingData, { transaction });

                    // Still stationary and previous gps isn't > 20 meters
                    await trackingRouteService.updateMovementStage(trackingData, { transaction });

                    // Stores current gps coordinates with tracking route id
                    await locationService.pushLatestLocation(activeWearable.id, latestCoordinates, activeTrackingRoute.id, { transaction });
                // OPTION B: If gps location is farther from reference point by (equal to or more than 20m away)
                    ...
                    // OPTION B: If current gps is > 20m and movement stage is 1 (meaning previous gps is also > 20m)
                        ...
                        await trackingRouteService.updateTrackingStatus(trackingData, { transaction });
                        ...
                        const newTrackingRoute = await trackingRouteService.addTrackingRoute(newTrackingData, { transaction });

                        // Reasigns first gps beyond >20m from stationary route to moving tracking route
                        await locationService.updateLocationTrackingRouteId(previousLocation.id, newTrackingRoute.id, { transaction });
                   
                        // Stores current gps coordinates with tracking route id
                        await locationService.pushLatestLocation(activeWearable.id, latestCoordinates, newTrackingRoute.id, { transaction });
                    }
                }
            // OPTION B
                ...
                if (distance >= MOVEMENT_THRESHOLD) {
                    ...
                    await trackingRouteService.updateStationaryObservationAt(trackingData, { transaction });
